...
#define BUILTIN_LED       D4     // GPIO2 Pin shared with UART1 TX
#define STARTUP_MODE_PIN  TX     // GPIO1 Pin for force default setup when LOW (using INPUT_PULLUP to force HIGH when disconnected)
...
#define SMOKE_PIN         D2     // GPIO4  Smoke trigger
#define SMOKE_PULSE_PIN   D1     // GPIO5  LED mirrors smoke pulses
#define FEEDBACK_PIN      D6     // GPIO12 PWM output showing DMX level to drive a LED intensity

SmokeController smoke(READY_PIN, READY_LED_PIN, SMOKE_PIN, SMOKE_PULSE_PIN, FEEDBACK_PIN);    // Use -1 to disable a feature
...
void checkInput(LXDMXWiFi* interface, WiFiUDP* iUDP, uint8_t multicast) {
  if ( got_dmx ) {
    ...
    for(int i=1; i<=got_dmx; i++) {     
      if (i == SMOKE_DMX_CHANNEL) {
        smoke.setDMXValue(ESP8266DMX.getSlot(i));
      }
...
void copyDMXToSmokeController(void) {
  uint8_t a, s;
  uint16_t a_slots = artNetInterface->numberOfSlots();
  uint16_t s_slots = sACNInterface->numberOfSlots();
  for (int i=1; i <=DMX_UNIVERSE_SIZE; i++) {
